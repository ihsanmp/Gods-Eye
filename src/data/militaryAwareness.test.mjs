...
test('pending mapped installations render as unknown instead of a false zero', async () => {
  ...
  try {
    runtime.dispatch('mm:awareness-subject-selected', awarenessSubject('flights', 'pending-site', position));
    await nextTurn();
...
test('a vessel feed still connecting after the lifecycle settles reads as unknown, not an all-clear', async () => {
  ...
  try {
    runtime.dispatch('mm:awareness-subject-selected', awarenessSubject('flights', 'connecting-site', position));
    await nextTurn();
...
test('a settled vessel feed reporting a real empty viewport recovers to 0', async () => {
  ...
  try {
    runtime.dispatch('mm:awareness-subject-selected', awarenessSubject('flights', 'settled-site', position));
    await nextTurn();
...
test('a still-answered feed keeps its count through later polls that set loading again', async () => {
  ...
  try {
    runtime.dispatch('mm:awareness-subject-selected', awarenessSubject('flights', 'refreshing-site', position));
    await nextTurn();
...
test('voice reports a still-connecting vessel feed as unknown rather than zero', async () => {
  ...
  try {
    runtime.dispatch('mm:awareness-subject-selected', awarenessSubject('flights', 'voice-site', position));
    await nextTurn();
...
test('tracked flight cleared during Contacts activation suppresses fallback autofocus', async () => {
  ...
  try {
    ...
    runtime.dispatch('mm:awareness-subject-cleared', {
      layerId: 'flights',
      id: 'selected',
    });
    releaseDependencies();
...
test('a fast-culled subject is reported absent so the readout can hold last-known', () => {
  ...
  try {
    ...
    militaryAwarenessLayer.setParams({ passive: false });
    runtime.dispatch('mm:awareness-subject-selected', {
...
test('an evicted vessel becomes CONTACT LOST instead of collapsing the panel', () => {
  ...
  try {
    ...
    runtime.dispatch('mm:entity-selection-cleared', {
...
test('a deliberate clear still fully clears the subject', () => {
  ...
  try {
    ...
    runtime.dispatch('mm:awareness-subject-cleared', { layerId: 'flights', id: 'ab4991' });
    assert.equal(
...
test('a deliberate source clear still fully clears the subject', () => {
  ...
  try {
    ...
    runtime.dispatch('mm:entity-selection-cleared', { layerId: 'ais-live-vessels' });
    assert.equal(militaryAwarenessLayer.getContextSnapshot(), null);
  } finally {
    restores.reverse().forEach((restore) => restore());
    runtime.restore();
...
test('user deselect cancels a pending Context entry auto-focus retry', async () => {
  ...
  try {
    ...
    runtime.dispatch('mm:entity-selection-cleared', { layerId: 'flights' });
...
test('same-layer selection clear is suppressed during a synchronous navigation reselect', () => {
  ...
  try {
    ...
    runtime.dispatch('mm:awareness-subject-selected', awarenessSubject('flights', 'a', positions.a));
...
test('FOCUS on an already-tracked subject clears its suppression keys before a later deselect', () => {
  ...
  try {
    ...
    runtime.dispatch('mm:awareness-subject-selected', awarenessSubject('flights', 'a', position));
...
test('NEXT jumps to expanded flight search after fully cycling nearby flight candidates', () => {
  ...
  try {
    ...
    runtime.dispatch('mm:awareness-subject-selected', awarenessSubject('flights', 'a', subjectPosition));
    assert.equal(militaryAwarenessLayer.navigateNext(), true);
    assert.equal(militaryAwarenessLayer.navigateNext(), true);
    assert.equal(militaryAwarenessLayer.navigateNext(), true);
...
test('NEXT can restrict navigation to a requested layer', () => {
  ...
  try {
    ...
    runtime.dispatch('mm:awareness-subject-selected', awarenessSubject('flights', 'f1', Cesium.Cartesian3.ZERO));
