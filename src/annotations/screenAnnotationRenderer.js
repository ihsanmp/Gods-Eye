...
export function createScreenAnnotationRenderer(viewer, {
  ...
  function add(anno) {
    const c = color(anno);
    const group = svgEl('g', { class: 'mm-anno', opacity: '0' });
    const parts = {};
    ...
    if (anno.type === 'area' && anno.ring && anno.ring.length >= 3) {
      ...
      parts.poly = svgEl('polygon', {
        class: anno.synthesized ? 'mm-anno-area' : 'mm-anno-area mm-draw',
        fill: c, stroke: c,
        'fill-opacity': anno.synthesized ? '0.08' : '0.14',
        'stroke-width': '2.5',
        ...(anno.synthesized ? { 'stroke-dasharray': '9 7' } : {}),
        filter: 'url(#mm-sketch)',
      });
      ...
      parts.path = svgEl('path', {
        class: 'mm-anno-arrow mm-draw', fill: 'none', stroke: c,
        'stroke-width': '3', 'marker-end': `url(#mm-arrow-${anno.color || 'primary'})`,
        filter: 'url(#mm-sketch)',
      });
      ...
      parts.dots = anno.path.map(() => {
        const dot = svgEl('circle', { class: 'mm-anno-dot', fill: c, stroke: '#06121c', 'stroke-width': '2', r: '4' });
       ...
       */
      parts.pin = svgEl('g', { class: 'mm-anno-pin' });
      ...
      parts.leader = svgEl('line', { class: 'mm-anno-leader', stroke: c, 'stroke-width': '1.5', 'stroke-opacity': '0.7' });
      ...
      if (anno.type !== 'label') {
        parts.ringOuter = svgEl('circle', { class: 'mm-anno-ring mm-pulse', fill: 'none', stroke: c, 'stroke-width': '2', r: '34' });
        parts.ringInner = svgEl('circle', { class: 'mm-anno-ring', fill: c, 'fill-opacity': '0.12', stroke: c, 'stroke-width': '2.5', r: '18', filter: 'url(#mm-sketch)' });
        group.appendChild(parts.ringOuter);
        group.appendChild(parts.ringInner);
      }
      parts.dot = svgEl('circle', { class: 'mm-anno-dot', fill: c, stroke: '#06121c', 'stroke-width': '2', r: anno.type === 'label' ? '4' : '5' });
    ...
    try {
      ...
      requestAnimationFrame(() => group.classList.add('mm-in'));
      ...
      for (const drawEl of group.querySelectorAll('.mm-draw')) {
  ...
  function projectAll() {
    ...
    for (const rec of records.values()) {
      ...
      // (The CSS `transition: opacity` was removed from .mm-anno so this per-frame JS ease
  ...
  function remove(anno) {
    const rec = records.get(anno.id);
    if (!rec) return;
    rec.group.classList.remove('mm-in');
    rec.group.classList.add('mm-out');
...
function buildOverlay() {
  ...
  layer.className = 'mm-screen-whiteboard';
  const svg = document.createElementNS(SVGNS, 'svg');
  svg.setAttribute('class', 'mm-screen-whiteboard-svg');
  svg.setAttribute('preserveAspectRatio', 'none');
  const defs = document.createElementNS(SVGNS, 'defs');
  // subtle hand-drawn wobble for strokes
  defs.innerHTML = `
    <filter id="mm-sketch" x="-20%" y="-20%" width="140%" height="140%">
...
function makeCallout(text, c) {
  ...
  node.setAttribute('class', 'mm-anno-callout');
  const rect = document.createElementNS(SVGNS, 'rect');
  rect.setAttribute('rx', '5');
  rect.setAttribute('class', 'mm-anno-card');
  const t = document.createElementNS(SVGNS, 'text');
  t.setAttribute('class', 'mm-anno-text');
  t.setAttribute('x', '9');
  t.setAttribute('y', '16');
  t.textContent = text;
  const accent = document.createElementNS(SVGNS, 'rect');
  accent.setAttribute('class', 'mm-anno-accent');
...
function ensureArrowMarker(defs, key, c) {
  if (defs.querySelector(`#mm-arrow-${key}`)) return;
  const marker = document.createElementNS(SVGNS, 'marker');
  marker.setAttribute('id', `mm-arrow-${key}`);
...
function injectStyles() {
  if (document.getElementById('mm-screen-whiteboard-styles')) return;
  const style = document.createElement('style');
  style.id = 'mm-screen-whiteboard-styles';
  style.textContent = `
  .mm-screen-whiteboard { position: fixed; inset: 0; pointer-events: none; z-index: 90; }
  .mm-screen-whiteboard-svg { position: absolute; inset: 0; width: 100%; height: 100%; overflow: visible; }
  /* No CSS opacity transition here on purpose: group opacity is driven per-frame in JS
     ...
     (flicker). Fade-OUT keeps its own transition via .mm-anno.mm-out below. */
  .mm-anno-ring { filter: drop-shadow(0 0 6px currentColor); }
  .mm-anno-dot { filter: drop-shadow(0 0 5px rgba(255,255,255,0.6)); }
  .mm-anno-area { filter: drop-shadow(0 0 5px currentColor); }
  .mm-anno-arrow { filter: drop-shadow(0 0 4px currentColor); stroke-linecap: round; }
  .mm-anno-leader { stroke-dasharray: 2 3; }
  .mm-anno-card { fill: rgba(8,18,28,0.78); stroke: rgba(255,255,255,0.14); stroke-width: 1; }
  .mm-anno-text { fill: #eaf6ff; font: 600 13px "JetBrains Mono", ui-monospace, monospace; letter-spacing: 0.02em; }
  /* draw-on: outlined shapes reveal their stroke */
  .mm-draw { stroke-dasharray: 1400; stroke-dashoffset: 1400; }
  .mm-anno.mm-in .mm-draw { transition: stroke-dashoffset 900ms ease-out; stroke-dashoffset: 0; }
  ...
  .mm-anno-callout { opacity: 0; }
  .mm-anno.mm-in .mm-anno-callout { animation: mm-pop 320ms ease-out both; }
  .mm-anno.mm-out { opacity: 0 !important; transition: opacity 320ms ease; }
  .mm-pulse { animation: mm-ring-pulse 1.8s ease-in-out infinite; transform-box: fill-box; transform-origin: center; }
  @keyframes mm-ring-pulse { 0%,100% { opacity: 0.9; stroke-width: 2; } 50% { opacity: 0.35; stroke-width: 3.5; } }
  @keyframes mm-pop { from { opacity: 0; } to { opacity: 1; } }
  `;
