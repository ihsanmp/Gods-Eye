...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => window.__mapMonitoring?.viewer && window.__mapMonitoring?.styleManager,
    ...
    const injected = await page.evaluate(({ fieldCounts, normalCounts }) => {
      const { viewer, dataManager, styleManager } = window.__mapMonitoring;
    ...
    await page.waitForFunction(
      () => window.__mapMonitoring.styleManager.getDetectionDiagnostics()?.observationCount === 5200,
      { timeout: SAMPLE_TIMEOUT_MS, polling: 100 },
    );
    await page.evaluate(() => {
      const viewer = window.__mapMonitoring.viewer;
